...
class DQNAgent:
    def __init__(self, action_size):
        ...
        # 변수들을 저장
        self.summary_writer = tf.summary.FileWriter('summary/mspacman_dqn', self.sess.graph)

        # 모든 형태를 지정한 후 다음 코드를 통해 변수를 초기화함
        self.sess.run(tf.global_variables_initializer())

        if self.load_model:
            self.model.load_weights("./save_model/mspacman_dqn.h5")
...
if __name__ == "__main__":
    ...
    for e in range(EPISODES):
        ...
        # 1000 에피소드마다 모델 저장
        if e % 1000 == 0:
            agent.model.save_weights("./save_model/mspacman_dqn.h5")
